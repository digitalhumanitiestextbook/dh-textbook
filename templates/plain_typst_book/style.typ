...
// PREFACE, 
  if preface != none {
    ...
    v(1em)
    set par(justify: true)
    align(center, box(width: 70%, text(11pt, overhang: true, font:  "Noto Sans CJK KR", fill: gray.darken(30%), preface)))
  }
...
// PAGE LAY OUT OF CONTENT
  ...
  // set code block style in pdf.
  show raw.where(block: true, lang: "python"): it => block(
    fill: luma(245),
    inset: (x: 10pt, y: 8pt),
    radius: 4pt,
  )[
    #set text(font: "D2Coding")
    #it
  ]
  show raw.where(block: true, lang: "text"): it => block(
    fill: luma(250),
    inset: (x: 10pt, y: 8pt),
    radius: 4pt,
  )[
  ...
  show raw.where(block: true, lang: "bash"): it => block(
    fill: luma(245),
    inset: (x: 10pt, y: 8pt),
    radius: 4pt,
  )[
  ...
  show raw.where(block: true, lang: "md"): it => block(
    fill: luma(245),
    inset: (x: 10pt, y: 8pt),
    radius: 4pt,
  )[
  ...
  show raw.where(block: true, lang: "yaml"): it => block(
    fill: luma(245),
    inset: (x: 10pt, y: 8pt),
    radius: 4pt,
  )[
  ...
  // Colab 버튼(HTML 블록) PDF에서 숨기기
  show raw.where(block: true): it => {
    // it.text 가 raw 내용(문자열)로 들어오는 형태를 기대
    if it.text.contains("colab-button:start") {
      // 아무것도 출력하지 않음
      none
    } else {
      it
